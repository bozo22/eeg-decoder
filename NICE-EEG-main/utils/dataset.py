...
def split_train_val(eeg_data, img_data, per_condition=False, split_ratio=0.05):
    ...
    Args:
        ...
        split_ratio: float - ratio of data to use for validation
    """
    ...
    if per_condition:
        ...
        val_nr_conditions = int(nr_conditions * split_ratio)
        ...
        for cond in val_conditions:
            start_idx = cond * SAMPLES_PER_CONDITION
            end_idx = start_idx + SAMPLES_PER_CONDITION
    ...
    val_eeg = torch.from_numpy(eeg_data[val_mask]).type(torch.FloatTensor)
    val_image = torch.from_numpy(img_data[val_mask]).type(torch.FloatTensor)
